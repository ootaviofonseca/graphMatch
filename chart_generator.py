...
def execute_code(code: str) -> str:
    ...
    #Removes the warnings to keep only the errors that actually cause problems
    errors_without_warnings = "\n".join(
    ...
    if process.returncode == 0:
        ...
        # Searches for the most recent SVG file inside the results folder
        svg_files = glob.glob(os.path.join(results_dir, "*.svg"))
        latest_svg = max(svg_files, key=os.path.getmtime) if svg_files else None

        return latest_svg
    else:
        return errors_without_warnings
...
class ChartGeneratorAgent:
    ...
    def __init__(self):
        ...
        self.code_tester = create_agent(
            ...
            tools=[execute_code],
            system_prompt= (
                "You receive Python code and execute it using the tool and return only the generated SVG "
                "Do NOT try to correct the code if there are errors. "
                "Just execute it and return only the tool output without adding anything "
            )
        )
    ...
    def generate_and_test_code(self, chart_type, question: str, summary: str,  path: str ) -> str:
            ...
            output = test_result["messages"][-1].content
            print(f"Meu output = {output}")
            """if "<svg" not in output.content.lower():
                correction_prompt = (
                    f"You developed the following {code}, and it produced the following errors: {output}. "
                    f"Fix the errors without changing the overall structure, the chart type, the data path, or the output format. "
                    f"You may adjust variable names, attributes, or logic if needed to make the code run correctly, "
                    f"but the chart type and general structure must remain the same. "
                    f"If it’s not possible to fix the code without changing these restricted parts, return the same input code exactly as it was."
                )
                code = self.code_generator.invoke({
